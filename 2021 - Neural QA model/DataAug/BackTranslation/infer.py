...
def bleuscore(model, eng_tokenizer, testX, en_sub_test):
    ...
    for i in range(int(len(a)/batch_size)):
        score = 0
        target, predicted = translate_batch(model, eng_tokenizer, a[(
            batch_size*i):(batch_size*(i+1))], b[(batch_size*i):(batch_size*(i+1))])
        for j in range(len(predicted)):
            score += sentence_bleu([clean(target[j]).split()],
                                   predicted[j].split(), weights=(0.25, 0.25, 0.25, 0.25))
