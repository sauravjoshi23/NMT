...
if __name__ == '__main__':
    ...
    parser.add_argument("setsize", type=int,
                        help="Count of datapoints for BackTranslation")
    ...
    # define model
    model = prepare_model.define_model(
        spr_vocab_size, eng_vocab_size, spr_length, eng_length, 256)
    model.compile(optimizer='Adam', loss='sparse_categorical_crossentropy')
    # summarize defined model
    print(model.summary())

    filename = args.path+'/BackTranslation/spen_sub20.h5'
    checkpoint = ModelCheckpoint(
        filename, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
    model.fit(trainX, trainY, epochs=50, batch_size=64, validation_data=(
        testX, testY), callbacks=[checkpoint], verbose=True)
    ...
    en_original, sp_original = infer.randomset(
        en_sub_train, sp_sub_train, setsize)
    ...
    for i in range(int(len(en_original)/batch)):
        t, batch_predictions = infer.translate_batch(model, eng_tokenizer, sp_infer[(
            batch*i):(batch*(i+1))], en_original[(batch*i):(batch*(i+1))])
        for prediction in batch_predictions:
            en_predicted.append(prediction)

    print('Back-Translation complete, saving to directory {}'.format(args.path+'/BackTranslation'))
    os.makedirs(args.path+'/BackTranslation', exist_ok=True)
    sp_bt = open(args.path+'/BackTranslation/bt.sparql', 'w')
    for s in sp_original:
        sp_bt.write(s + '\n')
    sp_bt.close()

    en_bt = open(args.path+'/BackTranslation/bt.en', 'w')
