    ...
    if (not prop[0] in prop_dic[original_count - count - 1]):
        for number in range(len(natural_language_question)):
            if count == original_count - 1 and device:
                ...
                final_quesition = pick_final_sentence(
                    original_question, final_candidates)
                advanced = pick_final_sentence_advanced(
                    device, original_question, final_candidates, bert_model_dir)
            ...
            if device and final_quesition:
                ...
                log.info(seperator.join(expand_line) + seperator +
                         str(rank) + seperator + "Paraphrased" + "\n")
        ...
        for number in range(len(natural_language_question)):
            if expanded_sparql_query:
                expand_line = [mother_ontology, "", "", expanded_sparql_query[number], expanded_sparql_query[number],
                               query_answer]
                expand_set.write((seperator.join(expand_line) +
                                 seperator + str(rank) + "\n").replace("  ", " "))
            vessel.append(
                [mother_ontology, "", "", natural_language_question[number], sparql_query[number], query_answer])
            test_set.write(
                (seperator.join(vessel[-1]) + seperator + str(rank) + "\n").replace("  ", " "))
            print("++++++++++++++++++++", vessel[-1], "+++++++++++++++")
