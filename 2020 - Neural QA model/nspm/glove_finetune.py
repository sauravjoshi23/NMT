...
def finetune_glove(project_path, glove_path="Glove/glove.6B.100d.txt", dimension=300):
    word_en = []
    with open(project_path+"/data.en", "r") as lines:
        for sentence in lines:
            ...
            for word in sentence.split():
                word_en.append(word.strip(":").strip(
                    "\"").strip("»").strip("+").strip("?"))
...
if __name__ == "__main__":
    ...
    if dimension:
        ...
        finetune_glove(path, "Glove/glove.6B." +
                       str(dimension)+"d.txt", dimension=dimension)
    else:
        finetune_glove(path, "Glove/glove.6B.100d.txt")
    pass
