...
def build_dataset_pair(binding, template):
    ...
    for variable in binding:
        ...
        # print("URI: ", uri)
        # PLACEHOLDER:  <A>
        # ENGLISH:  <A> has architected in how many cities?
        # SPARQL:  SELECT DISTINCT COUNT(?uri) where { ?x dbo:architect <A> . ?x dbp:locationTown ?uri . ?uri a dbo:City }
        # URI:  http://dbpedia.org/resource/D._H._Burnham_&_Company
    ...
    # SELECT DISTINCT COUNT attr_open var_uri attr_close where  brack_open  var_x dbo_architect dbr_D._H._Burnham_&_Company 
    # sep_dot var_x dbp_locationTown var_uri sep_dot var_uri a dbo_City  brack_close 
...
def get_results_of_generator_query(cache, template):
    ...
    for attempt, prepare_query in enumerate([first_attempt, second_attempt, third_attempt], start=1):
        ...
        #results -> {'a': {'type': 'uri', 'value': 'http://dbpedia.org/resource/Kevin_Roche'}, 'la': {'type': 'literal', 'value': 'Kevin Roche'}}
...
def prepare_generator_query(template, add_type_requirements=True, do_special_class_replacement=True):
    ...
    for i, variable in enumerate(variables):
        ...
        if variable_has_a_type and add_type_requirements:
            ...
            #print('NORMALIZED: ', normalized_target_class)
...
if __name__ == '__main__':
   ...
   # print use_resources_dump => False
    ...
    # templates output -> <generator_utils.Annotation object at 0x7f72247a1ee0>
