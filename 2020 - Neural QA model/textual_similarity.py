...
const.MODULE_URL = "https://tfhub.dev/google/universal-sentence-encoder-large/5"
...
def similarities(sentence, paraphrases):
    ...
    for v2 in vectors[1:]:
        cosine_similarities.append(cosine_similarity(
            np.array(vectors[0]), np.array(v2)))
    ...
    for i in range(len1 + 1):
        for j in range(len2 + 1):
            if i > 0 and j == 0:
                ...
                res1 = dp[i - 1][j] + 1
                res2 = dp[i][j - 1] + 1
                res3 = not s1[i - 1] == s2[j - 1] and dp[i -
                                                         1][j - 1] + 1 or dp[i - 1][j - 1]
...
if __name__ == "__main__":
    ...
    s1 = args.sentence1
    s2 = args.sentence2
    print("cosine similarity:", similarity(s1, s2),
