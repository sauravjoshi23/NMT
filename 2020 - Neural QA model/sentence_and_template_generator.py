...
def sentence_and_template_generator(prop_dic, test_set, log, mother_ontology, vessel, prop, project_name, output_file, diction, expand_set=[], tokenizer=None, device=None, model=None, original_count=0, count=0, suffix=" of <A> ?", query_suffix=""):
    ...
    for number in question_number:
        ...
        original_sparql = query_starts_with[number]+"where { <A>  " + \
            query_suffix + prop_link + " ?x " + query_ends_with[number]
    ...
    if(not prop[0] in prop_dic[original_count-count-1]):
        for number in range(len(natural_language_question)):
            if count == original_count-1 and device:
                ...
                final_quesiton = pick_final_sentence(
                    original_question, final_candidates)

                expanded_nl_question.append(final_quesiton)
                expanded_sparql_query.append(original_sparql)
            if expanded_sparql_query:
                expand_line = [mother_ontology, "", "", expanded_nl_question[number],
            ...
            output_file.write((';'.join(
                vessel[-1]) + ";" + str(rank) + ";" + "Original" + "\n").replace("  ", " "))
            log.info(';'.join(vessel[-1]) + str(rank) + "\n")

    else:
        for number in range(len(natural_language_question)):
            if expanded_sparql_query:
                expand_line = [mother_ontology, "", "", expanded_sparql_query[number],
                               expanded_sparql_query[number], query_answer]
                expand_set.write((';'.join(expand_line)+";" +
                                 str(rank)+"\n").replace("  ", " "))
            vessel.append([mother_ontology, "", "", natural_language_question[number],
                          sparql_query[number], query_answer])
            test_set.write(
    ...
    if(count > 0):
        ...
        for property_line in tqdm(list_of_property_information):
            ...
            sentence_and_template_generator(expand_set=expand_set, prop_dic=prop_dic, test_set=test_set, log=log, original_count=original_count, diction=diction, output_file=output_file,
                                            mother_ontology=mother_ontology, vessel=vessel, prop=prop_inside, suffix=suffix, count=count, project_name=project_name, query_suffix=query_suffix, tokenizer=tokenizer, device=device, model=model)
