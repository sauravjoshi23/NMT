...
def generate_templates(label, project_name, depth=1, output_file="basic_sentence_and_template_generator", paraphraser=False, multi=False, bert_model_dir=None):
    ...
    if multi:
        ...
        for property_line in list_of_property_information:
            ...
            print("**************\n"+str(prop))
            if paraphraser:
                basic_sentence_and_template_generator(original_count=depth, prop_dic=prop_dic, test_set=test_set, log=logger, diction=diction, output_file=output_file, mother_ontology=about.strip().replace(
                    "http://dbpedia.org/ontology/", "dbo:"), vessel=vessel, project_name=project_name, prop=prop, suffix=" of <A> ?", count=depth, expand_set=expand_set, tokenizer=tokenizer, device=device, model=model)
            else:
...
if __name__ == "__main__":
    ...
    generate_templates(label=label, project_name=project_name, depth=depth,
                       paraphraser=paraphraser, multi=multi, bert_model_dir=bert_model_dir)
    pass
