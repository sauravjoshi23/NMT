...
def generate_templates(label, project_name, depth=1, output_file="sentence_and_template_generator"):
    ...
    vessel = []
    depth = int(depth)
    diction = fetch_ranks("utility/part-r-00000.gz")
    ...
    list_of_property_information = get_properties(
        url=url, project_name=project_name, output_file="get_properties.csv")
    # list_of_property_information = []
    # with open('Person/get_properties.csv', 'r') as f:
    #     for line in tqdm(f, unit=' lines per '):
    #         x = line.split(',')
    #         x[3] = x[3][:-1]
    #         y = ','.join(x)
    #         list_of_property_information.append(y)
    # list_of_property_information = list_of_property_information[:14]
    for property_line in list_of_property_information:
        count += 1
        prop = property_line.split(',')
        print("**************\n"+str(prop))
        sentence_and_template_generator(original_count=depth, prop_dic=prop_dic, test_set=test_set, log=logger, diction=diction, output_file=output_file, mother_ontology=about.strip(
        ).replace("http://dbpedia.org/ontology/", "dbo:"), vessel=vessel, project_name=project_name, prop=prop, suffix=" of <A> ?", count=depth)
    output_file.close()
