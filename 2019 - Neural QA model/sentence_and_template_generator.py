...
def sentence_and_template_generator(prop_dic, test_set, log, mother_ontology, vessel, prop, project_name, output_file, diction, original_count=0, count=0, suffix=" of <A> ?", query_suffix=""):
    ...
    question_number = [2]
    # creating question and query templates based on property type
    ...
    # checking whether a valid prop_link exists
    ...
    for number in question_number:
        natural_language_question.append(
            question_starts_with[number]+prop[1] + suffix) # prop[0] = name, prop[1] = label
        sparql_query.append(
    ...
    # Check whether output for the query exists then only go for depth 2 queries else return with no output
    if(query_suffix == ""):
    ...
    rank = rank_check(diction=diction, count=count,
                      query=query_answer, original_count=original_count)

    # print("RANK: ", rank)
    # print("COUNT: ", count)
    # print("QUERY SUFFIX1: ", query_suffix)
    # RANK:  4.156286811685148e-05
    # COUNT:  1
    # QUERY SUFFIX1:  
    ...
    # print("COUNT2: ", count)
    # print("QUERY SUFFIX2: ", query_suffix)
    # COUNT2:  0
    # QUERY SUFFIX2:  dbo:agency ?x . ?x 

    # Writing nlq and sparql queries into the template sets
    if(not prop[0] in prop_dic[original_count-count-1]):
        # ("LEN: ", len(natural_language_question))
        for number in range(len(natural_language_question)):
            # print("MOTHER ONTOLOGY: ", mother_ontology)
            # print("natural_language_question[number]: ", natural_language_question[number])
            # print("sparql_query[number]: ", sparql_query[number])
            # print("query_answer: ", query_answer)

            # Example of a simple depth 1/2 pass
            # MOTHER ONTOLOGY:  dbo:Person
            # natural_language_question[number]:  What is the  alma mater of <A> ?
            # sparql_query[number]:  select ?x where { <A>  dbo:almaMater ?x  } 
            # query_answer:  select distinct(?a) where { ?a dbo:almaMater []  } 

            # Example of a recursive depth 2 pass
            # MOTHER ONTOLOGY:  dbo:Person
            # natural_language_question[number]:  Who is the  alumni of alma mater of <A> ?
            # sparql_query[number]:  select ?x where { <A>  dbo:almaMater ?x1 . ?x1 dbo:alumni ?x  } 
            # query_answer:  select distinct(?a) where { ?a dbo:almaMater [] . ?a  dbo:almaMater ?x1 . ?x1  dbo:alumni ?x } 
            
            vessel.append([mother_ontology, "", "", natural_language_question[number],
                          sparql_query[number], query_answer])
            output_file.write(
                (';'.join(vessel[-1])+";"+str(rank)+"\n").replace("  ", " "))
            log.info(';'.join(vessel[-1])+str(rank)+"\n")
    else: 
        # if a property i.e prop[0] gets repeated i.e if already present in the cycle of producing templates,
        # then it is added to the test set
        for number in range(len(natural_language_question)):
            vessel.append([mother_ontology, "", "", natural_language_question[number],
                          sparql_query[number], query_answer])
            test_set.write(
    ...
    suffix = " of " + prop[1] + " of <A> ?"  # changing suffix from "" to <-- as it will be passed to the recursive fn
    # Recursive Function for depth 2 sentences 
    # Property list generated again and csv file generated
    if(count > 0):
        ...
        for property_line in tqdm(list_of_property_information):
            ...
            print(prop_inside)
            sentence_and_template_generator(prop_dic=prop_dic, test_set=test_set, log=log, original_count=original_count, diction=diction, output_file=output_file,
                                            mother_ontology=mother_ontology, vessel=vessel, prop=prop_inside, suffix=suffix, count=count, project_name=project_name, query_suffix=query_suffix)
